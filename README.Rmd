...
The function `as.wt_dt` assigns the `wt_dt` to raw data set. It also allows you to specify the name of the raw variables corresponding to `panelist_id`, `url` and `timestamp`. Additionally, it turns the timestamp variable into `POSIXct` format.
...
- `classify_visits()` allows you to categorizes visits by either extracting the visit URL's
 domain or host and matching them to a list of domains or hosts; or by matching a list of regular expressions against the visit URL.
...
The package also contains functions to explore audience networks. More functionality will be added in later versions.
